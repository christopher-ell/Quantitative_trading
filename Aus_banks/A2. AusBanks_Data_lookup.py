...
def obtain_list_of_db_tickers():
    ...
    with con:
        cur = con.cursor()
        cur.execute("select id, ticker from symbol where currency = 'AUD' and sector = 'bank'")
        data = cur.fetchall()
        return [(d[0], d[1]) for d in data]
    

def get_daily_historic_data_yahoo_asx(ticker, sDate, eDate):
    ...
    try:
       ...
       prices_df = MYD.mod_Yahoo_downloader(ticker+'.ax', sDate, eDate).fillna(method='bfill')
       ...
       for index, row in prices_df.iterrows():
           daily_data.append(('ASX', ticker, index, row['Open'], row['High'], 
...
def example_sample_AusBank():
    # Set up
    con = connect_sec_db()
    cursor = con.cursor()
    #cursor.execute("show tables")
    #mysql_tbl_list = cursor.fetchall()
    cursor.execute("select * from symbol where currency = 'AUD' and sector = 'bank'")
    ...
    for t in tickers:
        ...
        yf_data = get_daily_historic_data_yahoo_asx(t[2], (2008,5,1), (2018,5,1))
        if not yf_data:
            print("#########################################")
