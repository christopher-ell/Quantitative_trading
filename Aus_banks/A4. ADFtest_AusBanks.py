...
price_data = pd.read_sql("select symbol_id, ticker, name, currency, price_date, adj_close_price from symbol inner join daily_price on symbol.id=daily_price.symbol_id where sector = 'Bank' and currency='AUD';", con=con)
